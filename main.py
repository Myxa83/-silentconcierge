...
async def raid_post(ctx, date, recruit_time, start_time, server, nickname, slots: int, channel_name: str):
    ...
    if not channel:
     ...
     embed = discord.Embed(
        ...
        f"📌 **Шепотіть:\n"
        f"```diff\n{nickname}\n```**\n\n"
        f"🧭 **Найм: {recruit_time} *(можу бути афк)* инагорода: буде роздаватись одразу, тому почекайте 5 хвилин після заходу й чекніть нагороду.**\n\n"
        ...
        f"🎫 Слотів: {slots}    ✅ Залишилось: {raid_data['slots'] - raid_data['taken']}"
    ),
    color=0x00ffcc
)
